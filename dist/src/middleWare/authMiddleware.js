...
async function login(email, password) {
    const employeeRepo = connection_1.dataSource.getRepository(userEntity_1.user);
    const users = await employeeRepo.findOneBy({ email });
    if (!users) {
        throw new Error('User not found');
    }
    // Compare hashed password using bcrypt
    const isPasswordValid = await bcryptjs_1.default.compare(password, users.password);
    if (!isPasswordValid) {
        throw new Error('Invalid password');
    }
    const role = users.role;
    const token = await (0, jwt_1.generateJwt)(users);
    return { token, role };
}
